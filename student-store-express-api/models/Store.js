...
class Storage {
  ...
  static getProductByEmail(email) {
    const productInfo = storage.get('purchases').find({ email }).value();
    if (!productInfo) {
      throw new BadRequestError(`Product with email ${email} not found`);
    }
    return productInfo;
  }
  ...
  static createReceipt(checkoutForm) {
    let lines = [
      `Showing receipt for ${checkoutForm.user.name} available at ${checkoutForm.user.email}:`,
    ];
    let totalPrice = 0;
    ...
    const productRows = checkoutForm.shoppingCart.map((item) => {
      ...
      lines.push(
        `${item.quantity} total ${productInfo.name} purchased at a cost of $${productInfo.price} for a total cost of $${itemTotalCost}`
      );
      return {
        ...productInfo,
    ...
    lines.push(
      `After taxes and fees were applied, the total comes out to $${this.round(
        totalPrice * 1.0875
      )}.`
    );
